...
void Touch::calibrate(Canvas &tft) {
  ...
  uint8_t state;
  uint16_t vxTL,vxTR,vxBR,vxBL,vyTL,vyTR,vyBR,vyBL,vt;
  int dx,dy;
	...
	if (touched(milliSeconds)) { 
		switch (state) {
 		 ...
 		 case 3:
			...
			//
			// TopLeft to TopRight 
			if (vxTL>vxTR) dx = (vxTL-vxTR); else dx = (vxTR-vxTL);
			if (vyTL>vyTR) dy = (vyTL-vyTR); else dy = (vyTR-vyTL);

			#define DEBUG
			#ifdef DEBUG
			char buffer[80];
			sprintf(buffer,"Calibrate dx:%d dy:%d",dx,dy);
			Serial.println(buffer);
			#endif			
			...
			if (dy > dx) {
				...
				vt=vxTL; vxTL=vyTL; vyTL=vt;
				vt=vxBR; vxBR=vyBR; vyBR=vt;
			}
			// 
			calibrateX(rc.left+5,vxTL,rc.right-5,vxBR);
			calibrateY(rc.top+5,vyTL,rc.bottom-5,vyBR);
			return;
		}
	}
  }
